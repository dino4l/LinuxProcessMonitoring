...
// структура, описывающая и хранящая данные о текущем соединении для каждого клиента
struct tcp_data_handler {
    struct sockaddr_in *address;
    struct socket *socket_accept;
    int thread_id;
};
// структура, описывающая и хранящая данные о текущих соединениях для всех клиентов
...
// ф-я записи в буфер сообщения, отправка с использованием сокетов на уровне ядра
int tcp_sending_from_server(struct socket *sock, int id, const char *buf, const size_t length, unsigned long flags)
{
    ...
    while (1) {
		...
		// пока есть что отправлять - отправляем
        if (len > 0) {
            written += len;
            left -= len;
...
// ф-я получения сообщения клиента
int tcp_receive_from_server(struct socket *sock, int id, struct sockaddr_in *address, unsigned char *buf, int size, unsigned long flags)
{
    ...
    vec.iov_len = size;
    vec.iov_base = buf;
	// есть очердь с сокетами клиентов,
	// пока очередь не пуста - из нее извлекается сокет клиента
    while (1) {
...
// Функция обработчик подключения, обработки запроса клиента:
// происходит вызов функции получения сообщения от клиента(tcp_receive_from_server) 
// и затем отправка ответа (tcp_sending_from_server)
int connection_handler(void *data)
{
    ...
    while (1) {
		...
		// проверка на то пуста ли очередь
        while (skb_queue_empty(&socket_accept->sk->sk_receive_queue)) {
            __set_current_state(TASK_INTERRUPTIBLE);
            schedule_timeout(HZ);
			// гарантирует, что потребительский поток не вернется немедленно в спящ. режим
...
// Ф-я для принятия новых соединений
// Определяет какой поток пустой, чтобы закинуть туда клиента: для этого создается
// сокета, очередь ожидания, новые клиенты обрабатываются в отдельном потоке
int tcp_accept_from_server(void)
{
    ...
    while (1) {
        ...
        add_wait_queue(&socket->sk->sk_wq->wait, &accept_wait); // созд. очередь ожидания

        while (reqsk_queue_empty(&isock->icsk_accept_queue)) { // если очердь запроса сокета пуста
            __set_current_state(TASK_INTERRUPTIBLE);
            schedule_timeout(HZ);
		...
		// новые клиенты обрабатываются в отдельном потоке
...
// Функция делает стандартную последовательность действий для любого сервера: 
// заполняет структуру sockaddr_in, вызывает фукнции bind и listen
// ф-я для принятия новых соед. к серверу: созд. сокета сервера, происх. связывание с портом
// Этот порт прослушивается для принятия новых соед. к серверу
...
// Изменяя регистр %rip — указатель на следующую исполняемую инструкцию,— мы изменяем инструкции, 
// которые исполняет процессор т.е. можем заставить его выполнить безусловный переход из текущей 
// функции в нашу. Таким образом мы перехватываем управление на себя. 
...
*	Если один из перехватов не удалось зарегестрировать, то все
